...
const HomeWork = () => {
  ...
  const handleMoreThan60 = () => {
    setProduct(data.filter((p) => p.price >= 60));
  };

  const handleMoreThan4 = () => {
    setProduct(data.filter((p) => p.rating.rate >= 4));
  };

  const handleFilterwomen = () => {
    setProduct(data.filter((p) => p.category === `women's clothing`));
  };
